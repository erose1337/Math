...
def generate_parameters(security_level=SECURITY_LEVEL, depth=DEFAULT_DEPTH, p=None):
    ...
    m_size = y_size / (depth + 1) # + 1 acommodates scalar multiplication
    ...
    for _outer in range(10):
        for count in range(1024):
            ...
            if PARAMETERS["depth"] == 2:
                ...
                c3 = encrypt(m3, key)
                c4 = encrypt(m4, key)
                s = random_integer(PARAMETERS["m_size"])
                mdot = decrypt(s * ((c * c2) + (c3 * c4)), key, depth=2)
                assert mdot == s * ((m * m2) + (m3 * m4)), (log(s, 2), log(((m * m2) + (m3 * m4)), 2), log(key[1], 2))
             #print count
        #print _outer
